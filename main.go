...
func printDevUsage() {
	...
	fmt.Println("  say <text> [--pipeline NAME] [--simulate]")
	fmt.Println("        Inject a synthetic transcript — matched commands REALLY execute")
	fmt.Println("        unless --simulate, which sinks every action dispatch on the chain")
	fmt.Println("        and reports what would have run (matcher tag writes still apply)")
	fmt.Println("  chain [tr_id] [--limit N] [--json]")
	fmt.Println("        Query correlated event chains (no id = recent-chains index)")
	fmt.Println("  events [--tr id] [--types GLOB[,GLOB]] [--plugin ID] [--severity warn] [--since 1h] [--source show-all|audit] [--limit N] [--json]")
