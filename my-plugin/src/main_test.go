...
func TestGreetDefault(t *testing.T) {
	...
	if result.ActionType() != "myplugin.greet" {
		t.Fatalf("expected action type %q, got %q", "myplugin.greet", result.ActionType())
	}
}
